...
const validateAndNormalizePayload = (payload) => {
  ...
  if (next.idNo !== undefined && next.idNo !== null && String(next.idNo).trim()) {
    const idNo = String(next.idNo).trim();
    if (!ID_REGEX.test(idNo)) {
      throw new Error("Validation error at idNo: expected letters/numbers only.");
    }
    next.idNo = idNo;
  }

  if (next.fullName !== undefined && next.fullName !== null && String(next.fullName).trim()) {
    const fullName = String(next.fullName).trim();
    if (!NAME_REGEX.test(fullName)) {
      throw new Error("Validation error at fullName: expected letters only.");
    }
    next.fullName = fullName;
  }

  if (next.contact !== undefined && next.contact !== null && String(next.contact).trim()) {
    const contact = String(next.contact).trim();
  ...
  if (next.educationLevel !== undefined && next.educationLevel !== null && String(next.educationLevel).trim()) {
    const edu = String(next.educationLevel).trim().toUpperCase();
    if (!EDUCATION_LEVELS.includes(edu)) {
      throw new Error(
        `Validation error at educationLevel: expected one of ${EDUCATION_LEVELS.join(", ")}.`
      );
    }
    next.educationLevel = edu;
    next.education = edu;
  }

  if (next.candidateStatus !== undefined && next.candidateStatus !== null && String(next.candidateStatus).trim()) {
    const candidateStatus = String(next.candidateStatus).trim().toUpperCase();
    if (!CANDIDATE_STATUSES.includes(candidateStatus)) {
      throw new Error(
        `Validation error at candidateStatus: expected one of ${CANDIDATE_STATUSES.join(", ")}.`
      );
    }
    next.candidateStatus = candidateStatus;
  }

  for (const numericField of ["trainingFee", "programFee"]) {
    if (next[numericField] !== undefined && next[numericField] !== null && next[numericField] !== "") {
      if (typeof next[numericField] !== "number" || Number.isNaN(next[numericField])) {
        throw new Error(
          `Validation error at ${numericField}: expected number but received ${typeof next[numericField]}.`
        );
      }
      const num = next[numericField];
      if (num < 0) {
        throw new Error(`Validation error at ${numericField}: must be >= 0.`);
      }
      next[numericField] = num;
    }
  }
...
export const upsertMyProfile = async (req, res) => {
  try {
    const payload = withDocumentStatuses(
      validateAndNormalizePayload(withProgramLogic({ ...req.body, userId: req.user.id }))
    );
...
export const updateMyProfile = async (req, res) => {
  try {
    const payload = withDocumentStatuses(validateAndNormalizePayload(withProgramLogic(req.body)));
    const profile = await CandidateProfile.findOneAndUpdate(
      { userId: req.user.id },
      payload,
      { new: true, runValidators: true }
...
export const upsertProfileByAdmin = async (req, res) => {
  try {
    ...
    const validatedPayload = validateAndNormalizePayload(payload);
    validatedPayload.idNo = await ensurePrefixedIdNo(CandidateProfile, validatedPayload.idNo, "CAN");
  ...
  try {
    ...
    for (let i = 0; i < rows.length; i += 1) {
      ...
      try {
        ...
        const ensuredIdNo = await ensurePrefixedIdNo(CandidateProfile, idNo, "CAN");
        const key = ensuredIdNo.toLowerCase();
        if (seenIdNos.has(key)) throw new Error("Duplicate idNo in uploaded file");
        seenIdNos.add(key);
        ...
        if (!userId && ensuredIdNo) {
          const existingProfile = await CandidateProfile.findOne({ idNo: ensuredIdNo });
          if (existingProfile?.userId) userId = existingProfile.userId;
        }
        if (!userId) {
          userId = await createCandidateUserIfMissing({
        ...
        const existingDifferent = await CandidateProfile.findOne({ idNo: ensuredIdNo, userId: { $ne: userId } });
        if (existingDifferent) throw new Error("Duplicate idNo already used by another candidate");
