...
const assertTrainingPayload = (payload) => {
  ...
  if (payload.trainingFee !== undefined && payload.trainingFee !== null && typeof payload.trainingFee !== "number") {
    throw new Error(`Validation error at trainingFee: expected number but received ${typeof payload.trainingFee}.`);
  }
};
...
export const createTrainingProgram = async (req, res) => {
  try {
    const payload = applyTrainingTypeDefaults(applyCandidateProgramLogic(normalizePayload(req.body)));
    payload.idNo = await ensurePrefixedIdNo(TrainingProgram, payload.idNo, "TR");
